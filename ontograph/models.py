...
class NodesDataframe:
    def __init__(self, terms, include_obsolete=False):
        self.dataframe = self.create_nodes_dataframe(terms, include_obsolete=include_obsolete)
        ...
        for annotation in annotations:
            ...
            # Get elements from LiteralPropertyValue annotations
                ...
                ann_dict[key] = {
                    ...
                    'datatype': self.__get_last_part_string(annotation.datatype)
                }            
        return ann_dict
    ...
    def create_nodes_dataframe(self, terms, include_obsolete=False):
        ...
        for term in tqdm(terms, desc="Building node dataframe", unit="term"):
            ...
            rows.append({
                ...
                "replaced_by": join([r.id for r in term.replaced_by]) if term.replaced_by else None,
                "consider": join(term.consider) if term.consider else None,
                "definition": str_(term.definition) if term.definition else None,
                "comment": term.comment,
                "annotations": str(get_ann(term.annotations)) if term.annotations else None,
                "subsets": join(term.subsets) if term.subsets else None,
                "synonyms": str(get_syn(term.synonyms)) if term.synonyms else None,
...
class EdgesDataframe:
    def __init__(self, terms, include_obsolete=False):
        self.dataframe = self.create_edges_dataframe(terms, include_obsolete=include_obsolete)
