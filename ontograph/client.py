...
class ClientOntology:
    ...
    #     backend = "pronto",
    # ) -> Ontology:
    #     """Load an ontology from a file path, an OBO Foundry identifier, or a URL.

    #     The method auto-detects the source type and uses the appropriate loading
    #     strategy:
    #      - "file": local filesystem path
    #      - "obo" : OBO Foundry ontology identifier (alphanumeric, underscore, hyphen)
    #      - "url" : http(s) URL

    #     Args:
    #         source (str): Local path, OBO id, or URL pointing to the ontology.
    #         downloader (DownloaderPort | None): Optional downloader to use when loading from URL.
    #         include_obsolete (bool): If True, include obsolete terms when building GraphBLAS structures.
    #         backend (str): Backend to use for subsequent queries: "pronto" (default) or "graphblas".

    #     Returns:
    #         Ontology: The loaded ontology object.

    #     Raises:
    #         ValueError: If the source type cannot be determined or an unknown backend is provided.
    #         RuntimeError: If loading fails for the detected strategy.

    #     Example:
    #         >>> client = ClientOntology()
    #         >>> ont = client.load("./tests/resources/dummy_ontology.obo")
    #         >>> isinstance(ont, Ontology)
    #         True
    #     """

    #     logger.info(f"Loading ontology from source: {source} ...")

    #     # Load ontology using the selected strategy
    #     try:
    #         logger.debug(f"Instantiating ProntoLoaderAdapter with cache_dir: {self._cache_dir}")
    #         loader = ProntoLoaderAdapter(cache_dir=self._cache_dir)
    #     except Exception as e:
    #         logger.error(f"Failed to instantiate ProntoLoaderAdapter: {e}")
    #         raise
    ...
    #         match strategy:
    #             case "file":
    #                 self._ontology = loader.load_from_file(file_path_ontology=source)
    #             case "obo":
    #                 self._ontology = loader.load_from_catalog(name_id=source, format="obo")
    #             case "url":
    #                 self._ontology = loader.load_from_url(source, Path(source).name, downloader)
    #             case _:
    #                 raise ValueError(f"Unknown ontology loading strategy: {strategy}")
    #     except Exception as e:
    #         raise RuntimeError(f"Failed to load ontology from '{source}' ({strategy}): {e}") from e
    ...
    #             ontology=self._ontology.get_ontology(),
    #             include_obsolete=include_obsolete
    #         )

    #     # Initialize query adapters
    #     self._initialize_queries(backend=backend)

    #     logger.info(f"Loading ontology from source: DONE")
